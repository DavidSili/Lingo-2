...
if ($grupe=="sve_reci") {
	...
	$grupeuslov=' AND grupa LIKE "%'.$grupe.'%" ';
	$grupeuslovb=' WHERE grupa LIKE "%'.$grupe.'%" ';
}
...
switch ($tipt) {
	case 1:
		$sql ='SELECT SQL_CALC_FOUND_ROWS *, r.ID rid
				...
				WHERE (t.ID IS NULL'.$grupeuslov.') OR ((t.ukupno < 4 OR t.ukupno IS NULL) AND  recnik="'.$recnik.'" AND smer="'.$smer.'")'.$insnaosnovu.$grupeuslov.'
				GROUP BY r.ID
				ORDER BY RAND()
				LIMIT '.$klk;
		break;
	case 2:
		$sql ='SELECT SQL_CALC_FOUND_ROWS *, r.ID rid
				FROM '.$recnik.' AS r
				 LEFT JOIN test_reci AS t ON r.ID = t.idreci
				WHERE (t.procenat >='.$min.' AND t.procenat <'.$himin.') AND recnik="'.$recnik.'" AND smer="'.$smer.'" '.$insnaosnovu.$grupeuslov.'
				GROUP BY r.ID
				ORDER BY RAND()
				LIMIT '.$klk;
		break;
	case 3:
		$sql ='SELECT SQL_CALC_FOUND_ROWS *, r.ID rid
				FROM '.$recnik.' AS r
				 LEFT JOIN test_reci AS t ON r.ID = t.idreci
				WHERE (t.procenat >='.$himin.' AND t.procenat <'.$avg.') AND recnik="'.$recnik.'" AND smer="'.$smer.'" '.$insnaosnovu.$grupeuslov.'
				GROUP BY r.ID
				ORDER BY RAND()
				LIMIT '.$klk;
		break;
	case 4:
		$sql ='SELECT SQL_CALC_FOUND_ROWS *, r.ID rid
				FROM '.$recnik.' AS r
				 LEFT JOIN test_reci AS t ON r.ID = t.idreci
				WHERE (t.procenat >='.$avg.' AND t.procenat <'.$lomax.') AND recnik="'.$recnik.'" AND smer="'.$smer.'" '.$insnaosnovu.$grupeuslov.'
				GROUP BY r.ID
				ORDER BY RAND()
				LIMIT '.$klk;
		break;
	case 5:
		$sql ='SELECT SQL_CALC_FOUND_ROWS *, r.ID rid
				FROM '.$recnik.' AS r
				 LEFT JOIN test_reci AS t ON r.ID = t.idreci
				WHERE (t.procenat >='.$lomax.' AND t.procenat <='.$max.') AND recnik="'.$recnik.'" AND smer="'.$smer.'" '.$insnaosnovu.$grupeuslov.'
				GROUP BY r.ID
				ORDER BY RAND()
				LIMIT '.$klk;
		break;
	...
	case 9:
		...
		$sql='SELECT *, r.ID rid
				FROM '.$recnik.' AS r
				 LEFT JOIN test_reci AS t ON r.ID = t.idreci
				WHERE t.datum <"'.$premesec.'"'.$insnaosnovu.' AND recnik="'.$recnik.'" AND smer="'.$smer.'" '.$insnaosnovu.$grupeuslov.'
				GROUP BY r.ID
				ORDER BY RAND()
				LIMIT '.$klk;
		break;
	case 10:
		...
		$sql='SELECT *, r.ID rid
				FROM '.$recnik.' AS r
				 LEFT JOIN test_reci AS t ON r.ID = t.idreci
				WHERE t.datum <"'.$presedmicu.'"'.$insnaosnovu.' AND recnik="'.$recnik.'" AND smer="'.$smer.'" '.$insnaosnovu.$grupeuslov.'
				GROUP BY r.ID
				ORDER BY RAND()
				LIMIT '.$klk;
		break;
	case 11:
		$sql ='SELECT SQL_CALC_FOUND_ROWS *, ID rid FROM '.$recnik.$grupeuslovb.' ORDER BY RAND() LIMIT '.$klk;
		break;
}
// $debug=$sql;
